...
# === Load & Process Data ===
uploaded_file = st.file_uploader("Upload your Daily View CSV", type=["csv"])

@st.cache_data
def load_data(file):
    df_clean = pd.read_csv(file, encoding='utf-8-sig')
    ...
    survival_df['duration'] = survival_df.apply(
        lambda row: row['event_day'] if row['event_occurred'] else row['last_day'], axis=1)
...
# Only proceed if user uploads a file
if uploaded_file:
    survival_df = load_data(uploaded_file)
else:
